...
import { addStokMasuk, updateStokMasuk } from "@/lib/stokMasukApi";
...
function extractDateFromISO(isoString) {
  ...
  try {
    ...
    // Convert ISO format to local date, accounting for timezone
...
export default function StokMasukForm({ mode = "add", initialData = null }) {
  ...
  async function onSubmit(event) {
    ...
    const confirmResult = await Swal.fire({
      title: isEditMode ? "Simpan perubahan transaksi?" : "Tambah stok masuk?",
      text: isEditMode
        ? "Perubahan akan menyesuaikan stok pada master barang."
        : "Stok barang pada master akan bertambah otomatis.",
      icon: "question",
    ...
    try {
      ...
      if (isEditMode) {
        await updateStokMasuk(initialData.id, payload);
        await Swal.fire({
          title: "Berhasil",
          text: "Transaksi stok masuk berhasil diperbarui.",
          icon: "success",
          confirmButtonText: "OK",
          confirmButtonColor: "#2563eb",
        });
      } else {
        await addStokMasuk(payload);
        await Swal.fire({
          title: "Berhasil",
          text: "Transaksi stok masuk berhasil ditambahkan.",
          icon: "success",
          confirmButtonText: "OK",
          confirmButtonColor: "#2563eb",
        });
      ...
      setError(err.message || `Gagal ${isEditMode ? "memperbarui" : "menambahkan"} transaksi stok masuk.`);
    } finally {
  ...
  return (
    <section className="rounded-lg border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center gap-3 border-b border-gray-200 px-6 py-4">
        ...
        <div>
          <h3 className="text-2xl font-bold text-gray-900">{isEditMode ? "Edit Stok Masuk" : "Tambah Stok Masuk"}</h3>
          <p className="mt-1 text-sm text-gray-600">
            {isEditMode
              ? "Perbarui transaksi dan sesuaikan stok secara otomatis."
              : "Tambahkan transaksi stok masuk baru."}
          </p>
        </div>
      </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <FormField label="Kode Barang" required>
              <Select
                ...
                className="text-sm"
                classNamePrefix="stok-masuk-select"
                styles={{
                  control: (base, state) => ({
                    ...base,
                    minHeight: 42,
