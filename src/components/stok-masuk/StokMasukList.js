...
import { deleteStokMasuk, getStokMasuk } from "@/lib/stokMasukApi";
...
export default function StokMasukList() {
  ...
  async function loadData() {
    ...
    try {
      const stokMasukData = await getStokMasuk();
      setTransactions(stokMasukData);
    } catch (err) {
      setError(err.message || "Gagal memuat data stok masuk.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (term) {
      result = result.filter((item) =>
        [item.tanggal, item.kode_barang, item.nama_barang, item.supplier, item.keterangan]
          .filter(Boolean)
  ...
  async function onDelete(id) {
    ...
    const result = await Swal.fire({
      ...
      text: "Stok master akan dikurangi otomatis sesuai jumlah transaksi ini.",
      icon: "warning",
    ...
    try {
      await deleteStokMasuk(id);
      await Swal.fire({
        title: "Berhasil",
        text: "Transaksi stok masuk berhasil dihapus.",
        icon: "success",
      ...
      setError(err.message || "Gagal menghapus transaksi stok masuk.");
    }
  }
    ...
    <section className="rounded-lg border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-200 p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h3 className="text-2xl font-bold text-gray-900">Stok Masuk</h3>
            <p className="mt-1 text-sm text-gray-600">Kelola transaksi stok masuk.</p>
          </div>
      ...
      <div className="p-6">
        <div className="mt-2 mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 sm:flex-1 sm:max-w-xs">
            <input
              type="text"
              placeholder="Cari tanggal, kode, nama, supplier..."
              value={searchTerm}
              ...
              {!isLoading && filteredTransactions.length === 0 && (
                <tr>
                  <td colSpan={8} className="border-t border-gray-200 py-12 text-center text-gray-500">
                    {searchTerm ? "Tidak ada data yang cocok." : "Belum ada data stok masuk."}
                  </td>
                </tr>
              )}
